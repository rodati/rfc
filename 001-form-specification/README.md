...
type | `string` | **REQUIRED** It is a string that is defined by the service and represents the data type that will be sent in as data. These are the available types: `text`, `number`, `time`, `submitAndRefreshButton`, `actionButton`, `multiCheck`, `check`, `select`, `password`, `multiText`, `multiRadio`
